...
public static class RealWorldExamples
{
    public static async Task RunAllAsync(SampleDbContext db, ILogger logger)
    {
        ...
        await Example5_SmartMeterSync(db, logger);
    }
    ...
    private static async Task Example1_InventoryRestock(SampleDbContext db, ILogger logger)
    {
        ...
        logger.LogInformation(
            "Created {Count} products suffix={Suffix}",
            products.Length,
            suffix);
        ...
        try
        {
            ...
            var restockMap = new Dictionary<string, int>
            {
                [skus[0]] = 50,
                [skus[1]] = 100,
                [skus[2]] = 200
            ...
            var result = await db.BulkExecuteAsync(batch =>
            {
                foreach (var (sku, qty) in restockMap)
                {
                    batch.Update<Product>(op => op
                        .Where(p => p.Sku == sku)
                        .Set(p => p.StockQuantity, p => p.StockQuantity + qty)
                        .Set(p => p.IsActive, true)
                        .Set(p => p.LastRestocked, restockDate));
                }
            });
            ...
            logger.LogInformation(
                "Restock completed Products={Rows} Logs={Logs}",
                result.TotalRowsAffected,
                restockMap.Count);

            db.ChangeTracker.Clear();

            var restocked = await db.Products
                .AsNoTracking()
                .Where(p => skus.Contains(p.Sku))
                .ToListAsync();

            foreach (var p in restocked)
            {
                logger.LogInformation(
                    "{Sku} Stock={Stock} Active={Active}",
                    p.Sku,
                    p.StockQuantity,
                    p.IsActive);
            }
        ...
        {
            await tx.RollbackAsync(); logger.LogError(ex, "Restock failed");
        }
    ...
    private static async Task Example2_OrderFulfillment(SampleDbContext db, ILogger logger)
    {
        ...
        logger.LogInformation(
            "Created {Count} pending orders suffix={Suffix}",
            orders.Length,
            suffix);
        ...
        try
        {
            ...
            var result = await db.BulkExecuteAsync(batch =>
            {
                foreach (var n in orderNumbers)
                {
                    batch.Update<Order>(op => op
                        .Where(o => o.OrderNumber == n && o.Status == OrderStatus.Pending)
                        .Set(o => o.Status, OrderStatus.Shipped)
                        .Set(o => o.ShippedDate, shippedDate));
                }

                batch.Update<Customer>(op => op
                    .Where(c => c.Id == customer.Id)
                    .Set(c => c.LoyaltyPoints, c => c.LoyaltyPoints + 20)
                    .Set(c => c.LastOrderDate, shippedDate));

                batch.Update<Product>(op => op
                    .Where(p => p.Sku == product1.Sku)
                    .Set(p => p.StockQuantity, p => p.StockQuantity - 1));

                batch.Update<Product>(op => op
                    .Where(p => p.Sku == product2.Sku)
                    .Set(p => p.StockQuantity, p => p.StockQuantity - 1));
            });
            ...
            logger.LogInformation(
                "Fulfillment ops={Count} rows={Rows}",
            ...
            var shipped = await db.Orders
                .CountAsync(o => orderNumbers.Contains(o.OrderNumber)
                    && o.Status == OrderStatus.Shipped);

            var upd = await db.Customers
                .AsNoTracking()
                .FirstAsync(c => c.Id == customer.Id);

            logger.LogInformation(
                "Shipped={Shipped} Points={Points}",
                shipped,
                upd.LoyaltyPoints);
        }
        catch (Exception ex)
        {
            ...
            logger.LogError(ex, "Fulfillment failed");
        }
    ...
    private static async Task Example3_PriceUpdate(SampleDbContext db, ILogger logger)
    {
        ...
        {
            batch.Update<Product>(op => op
                .Where(p => p.Category == catApparel && p.IsActive)
                .Set(p => p.Price, p => p.Price * 0.8m));

            batch.Update<Product>(op => op
                .Where(p => p.Category == catElec && p.IsActive)
                .Set(p => p.Price, p => p.Price * 0.9m));

            batch.Update<Product>(op => op
                .Where(p => p.Category == catAcc && p.IsActive)
                .Set(p => p.Price, p => p.Price * 0.95m));
        });

        logger.LogInformation(
            "Price updates ops={Ops} rows={Rows}",
            result.Operations.Count,
            result.TotalRowsAffected);
        ...
        foreach (var g in updated.GroupBy(p => p.Category))
        {
            logger.LogInformation("Category {Key}", g.Key);

            foreach (var p in g)
            {
                logger.LogInformation(" {Name}: {Price}", p.Name, p.Price);
            }
        }
    }
    ...
    private static async Task Example4_CustomerLoyaltyBatch(SampleDbContext db, ILogger logger)
    {
        ...
        logger.LogInformation(
            "Created {Count} customers suffix={Suffix}",
            customers.Length,
            suffix);
        ...
        {
            batch.Update<Customer>(op => op
                .Where(c => c.Email.Contains(suffix)
                    && c.LoyaltyPoints >= 100
                    && c.LoyaltyPoints < 500
                    && c.IsActive)
                .Set(c => c.LoyaltyPoints, c => c.LoyaltyPoints + 50));
            ...
            batch.Update<Customer>(op => op
                .Where(c => c.Email.Contains(suffix)
                    ...
                    && c.IsActive)
                .Set(c => c.LoyaltyPoints, c => c.LoyaltyPoints + 100));
            
            batch.Update<Customer>(op => op
                .Where(c => c.Email.Contains(suffix)
                    && c.LoyaltyPoints >= 1000
                    && c.IsActive)
                .Set(c => c.LoyaltyPoints, c => c.LoyaltyPoints + 200));

            batch.Update<Customer>(op => op
                .Where(c => c.Email.Contains(suffix)
                    && c.LastOrderDate != null
                    && c.LastOrderDate < sixMonthsAgo
                    && c.IsActive)
        ...
        {
            var tier = c.LoyaltyPoints >= 1000 ? "Gold" : c.LoyaltyPoints >= 500 ? "Silver" : c.LoyaltyPoints >= 100 ? "Bronze" : "New";
            
            logger.LogInformation(
                "{Name}: {Tier} {Points} Active={Active}",
                c.Name,
                tier,
                c.LoyaltyPoints,
                c.IsActive);
    ...
    private static async Task Example5_SmartMeterSync(SampleDbContext db, ILogger logger)
    {
        logger.LogInformation("Example 5: Smart Meter Sync (upsert batch)");
        var suffix = Guid.NewGuid().ToString("N")[..6];
        var today = DateTime.UtcNow.Date;
        var existingMeter = $"MTR-{suffix}-1";
        var newMeter = $"MTR-{suffix}-2";

        // Yesterday's state: MTR-...-1 already reported 12.00 kWh today.
        db.EnergyReadings.Add(new EnergyReading
        {
            MeterId = existingMeter,
            Date = today,
            ConsumptionKwh = 12.00,
        ...
        // Nightly sync, one round-trip: refresh the known meter, insert the new one.
        // Match refreshes only ConsumptionKwh + RecordedAt; everything else is untouched.
        var result = await db.BulkExecuteAsync(batch =>
        {
            var known = new EnergyReading
            {
                MeterId = existingMeter,
                Date = today,
            ...
            batch.Upsert<EnergyReading>(op => op
                .MatchOn(r => new { r.MeterId, r.Date })
                .Update(r => r.ConsumptionKwh, known.ConsumptionKwh)
                .Update(r => r.RecordedAt, known.RecordedAt)
                .Insert(known));

            var fresh = new EnergyReading
            {
                MeterId = newMeter,
                Date = today,
                ConsumptionKwh = 7.25,
            ...
            batch.Upsert<EnergyReading>(op => op
                .MatchOn(r => new { r.MeterId, r.Date })
                .Update(r => r.ConsumptionKwh, fresh.ConsumptionKwh)
                .Update(r => r.RecordedAt, fresh.RecordedAt)
                .Insert(fresh));
        });

        logger.LogInformation(
            "Meter sync ops={Ops} rows={Rows}",
            result.Operations.Count,
            result.TotalRowsAffected);

        db.ChangeTracker.Clear();

        var meters = new[] { existingMeter, newMeter };
        
        var readings = await db.EnergyReadings
            .AsNoTracking()
            .Where(r => meters.Contains(r.MeterId) && r.Date == today)
            .OrderBy(r => r.MeterId)
            .ToListAsync();

        foreach (var r in readings)
        {
            logger.LogInformation("{Meter} {Kwh} kWh", r.MeterId, r.ConsumptionKwh);
        }
    }
}
