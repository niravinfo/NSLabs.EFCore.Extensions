...
| `decimal` | MUST use the column's configured precision/scale (`IProperty.GetPrecision()/GetScale()`); fallback `decimal(38,18)` only when unconfigured |
| `double` | `float` |
| `float` (`Single`) | `real` |
| `TimeSpan` | `time` |
| `char` | `nchar(1)` (or the column length — mirror what EF emits) |
| `string` | `nvarchar(max)` always — never the column length (a `WITH (nvarchar(n))` would truncate over-long inputs and false-match rows the exact value could never equal) |
