...
- **Switch to the single-param fast path when EITHER is true:**
  1. `nonNullCount > Threshold` (the "mark"), **OR**
...
{
    if (nonNullCount == 0)
        return false;   // empty (1=0/1=1) and all-null (IS NULL/IS NOT NULL) cost 0 params; never fast path
...
- `otherParamsInOp`: assignments + other predicate parts + discriminator (already counted today).
- **Multi-`IN` ops — sequential decision in predicate order (locked):** the backstop needs the op-level total, so compute op cost in two passes — first sum all non-`IN` params into `other`, then visit each `SqlInNode` in predicate order, deciding with `other + sum(decided IN costs so far)` as the running `otherParamsInOp`. Each decided fast-path `IN` contributes 1 (0 if `nonNullCount == 0`), each slow-path `IN` contributes its `nonNullCount`. This keeps counting and emission order-identical. Example: two 1200-id lists with budget 2000 and `Threshold = int.MaxValue`: first sees `1200 + other > 2000` → fast (cost 1); second sees `1200 + other + 1 > 2000` → fast. A single `IN` of 5000 ids with 1 assignment: `other = 1`, `5000 + 1 > 2000` → fast path → op cost becomes `1 + 1 = 2` (nulls stripped pre-count: a 5000+null list costs the same 1 param + `OR IS NULL` with 0 params).
- **Upsert-guard context (locked):** `CountParameterNodes(spec.Guard)` is fast-aware with `other = assignment costs + perRowCost` for a single row (i.e. the guard's backstop sees the worst-case single-row unit: `fixed + perRowCost > effectiveLimit` still throws). The guard's decided cost becomes the `fixedCost` used by `ExpandUpsert` capacity math. The JSON/array payload is re-emitted per row-slice chunk (duplicated wire cost — accepted for v1, see §7 item 7).
...
3. **New** `Internal/LargeListHelper.cs` (core, provider-neutral): SQL Server/SQLite `Threshold` consts + `ShouldUseFastPath(nonNullCount, effectiveLimit, otherParamsInOp, threshold)` with the `nonNullCount == 0 → false` guard (§4.1) + sequential multi-`IN` application order, null-strip helper (`PartitionNulls` → `(nonNulls, hasNull)`), JSON payload builder via `System.Text.Json` over converted **non-nulls**, typed non-nullable array builder for PG (`Array BuildTypedArray(IProperty, nonNulls)`; `byte[]` elements excluded → caller keeps multi-param route — for PG that means plain `IN (@p…)` with normal budget checks).
